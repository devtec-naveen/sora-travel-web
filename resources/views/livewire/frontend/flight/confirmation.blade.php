                    ...
                    <div class="card p-4 md:p-6 flex flex-col items-center text-center gap-8">
                        ...
                        <div class="flex flex-col items-center gap-6">
                            ...
                            <div class="flex flex-col gap-2">
                                <h1 class="font-semibold text-xl sm:text-2xl md:text-3xl lg:text-4xl leading-tight text-slate-900">
                                    Booking Confirmed!
                                </h1>
                                <p class="font-normal text-base text-slate-500 max-w-md mx-auto">
                                    Your booking is confirmed. The official ticket and payment receipt is sent to your registered email.
                                </p>
                            </div>
                        </div>
                        {{-- Booking ID --}}
                        @if (!empty($order['id']))
                            <div class="w-full bg-blue-50 p-3 md:p-5 rounded-2xl border border-blue-200 flex flex-col gap-1.5">
                                <span class="font-normal text-sm md:text-base text-slate-500 uppercase tracking-wider">Booking ID</span>
                                <span class="font-bold text-xl sm:text-2xl md:text-3xl leading-none text-blue-600 break-all">
                                    {{ strtoupper($order['id']) }}
                                </span>
                            </div>
                        @endif
                        {{-- Flight Details — all slices --}}
                        @if (!empty($slices))
                            <div class="w-full flex flex-col gap-8 text-left">
                                <h2 class="font-semibold text-xl text-slate-950">
                                ...
                                @foreach ($slices as $sliceIdx => $slice)
                                    @php
                                        ...
                                        if ($tripType === 'multi_city') {
                                            $sliceLabel = 'Flight ' . ($sliceIdx + 1) . ' · ' . $orig . ' → ' . $dest;
                                        } elseif (count($slices) > 1) {
                                            $sliceLabel = $sliceIdx === 0 ? 'Outbound Flight' : 'Return Flight';
                                        } else {
                                            $sliceLabel = null;
                                        }
                                    @endphp

                                    <div class="flex flex-col gap-5 @if(!$loop->last) pb-6 border-b border-slate-100 @endif">
                                        ...
                                        @if ($sliceLabel)
                                            <div class="flex items-center gap-2">
                                                <span class="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-blue-50 border border-blue-200 text-blue-700 text-xs font-semibold">
                                                    <i data-tabler="plane-departure" data-size="12"></i>
                                                    {{ $sliceLabel }}
                                                </span>
                                            </div>
                                        @endif
                                        ...
                                        <div class="flex items-center gap-4">
                                            ...
                                            <div class="flex flex-col gap-0.5">
                                                ...
                                                <span class="font-normal text-sm text-slate-500 tracking-wide">{{ $fno }}</span>
                                            </div>
                                        </div>
                                        ...
                                        <div class="flex flex-row items-center justify-between gap-4">
                                            <div class="flex flex-col items-start">
                                                ...
                                                <span class="font-normal text-sm text-slate-500">{{ $origCity }} ({{ $orig }})</span>
                                                @if ($dep)
                                            ...
                                            <div class="flex flex-col items-end">
                                                ...
                                                <span class="font-normal text-sm text-slate-500 text-right">{{ $destCity }} ({{ $dest }})</span>
                                                @if ($arr)
                        ...
                        <div class="w-full flex flex-col gap-4 text-left">
                            ...
                            <div class="space-y-3">
                                @foreach ([
                                    ...
                                    'Make sure to bring a valid passport and/or government-issued ID.',
                                    'You can check in online 24 hours before your flight.',
                                    'Baggage allowance: 1 checked bag (23 kg) + 1 carry-on (7 kg).',
                                ] as $info)
                                    <div class="flex gap-3">
                                        <i data-tabler="check" class="text-green-600 shrink-0 mt-0.5" data-size="16"></i>
                                        <span class="font-normal text-sm text-slate-700 leading-relaxed">{{ $info }}</span>
                                    </div>
                                @endforeach
                            </div>
                        </div>
