        ...
        <div class="booking-progress-container py-6">
            <div class="container">
                <div class="flex items-center justify-between max-w-5xl mx-auto">
                    ...
                    <div class="grow h-0.5 bg-blue-600 rounded-full"></div>
                    <div class="flex flex-col items-center gap-2 shrink-0">
                        <div class="sm:w-8 sm:h-8 w-6 h-6 rounded-full bg-blue-700 flex items-center justify-center text-white shrink-0 shadow-sm shadow-blue-100 font-bold text-sm">4</div>
                ...
                <div class="flex flex-col lg:flex-row gap-8 lg:items-start">
                    <div class="flex-1 flex flex-col gap-6">
                        <div class="flex flex-col items-center lg:items-start gap-2.5 px-4 lg:px-0">
                            ...
                            <span class="font-normal text-base text-slate-500">Travelers under the age of 2 must sit on an adult's lap</span>
                        </div>
                        @if ($fetchError)
                            <div class="card p-5 flex items-start gap-3 border border-red-100 bg-red-50">
                                ...
                                <div>
                                    ...
                                    <p class="text-sm text-red-500 mt-1">Could not fetch seat maps from the airline. You can skip seat selection and continue.</p>
                                </div>
                        </div>
                        @elseif ($noSeatsAvailable)
                            <div class="card p-5 flex items-start gap-3 border border-amber-100 bg-amber-50">
                                ...
                                <div>
                                    ...
                                    <p class="text-sm text-amber-600 mt-1">This airline does not support seat selection through our platform. Seats will be assigned at check-in.</p>
                                </div>
                            </div>
                        @else
                            @if (count($seatMaps) > 1)
                                    ...
                                    <div class="flex gap-2 flex-wrap">
                                        @foreach ($passengerMeta as $paxKey => $meta)
                                            ...
                                            <button wire:key="tab-pax-{{ $paxKey }}"
                                                ...
                                                {{ $meta['name'] }}
                                                @if ($pSeat)
                                                    <span wire:key="badge-{{ $paxKey }}" class="px-1.5 py-0.5 rounded-md bg-[#f3b515] text-white text-xs font-bold">
                                                        {{ $pSeat['designator'] }}
                                                    </span>
                                                @endif
                                            </button>
                                        ...
                                        <div class="flex flex-col xl:flex-row gap-8">
                                            <div class="flex-1 overflow-x-auto pb-2">
                                                @foreach ($currentMap['cabins'] ?? [] as $cabinIdx => $cabin)
                                                    @if (count($currentMap['cabins']) > 1)
                                                        <div class="mb-4">
                                                            <span class="text-xs font-bold uppercase tracking-widest text-slate-400 bg-slate-100 px-3 py-1 rounded-full">
                                                    ...
                                                    <div class="inline-block min-w-max">
                                                        <div class="flex items-center gap-1 mb-2 pl-10">
                                                            @foreach ($headerSections as $secIdx => $section)
                                                                ...
                                                                @foreach ($section['elements'] ?? [] as $el)
                                                                    @if (($el['type'] ?? '') !== 'seat') @continue @endif
                                                                    @php $letter = preg_replace('/[0-9]/', '', $el['designator'] ?? ''); @endphp
                                                                    <div class="w-9 h-7 flex items-center justify-center shrink-0">
                                                                        <span class="font-semibold text-sm text-slate-700">{{ $letter }}</span>
                                                                    </div>
                                                                @endforeach
                                                            @endforeach
                                                        </div>
                                                        @foreach ($cabin['rows'] ?? [] as $rowIdx => $row)
                                                            @php
                                                                ...
                                                                $allElements = collect($sections)->flatMap(fn($s) => $s['elements'] ?? []);
                                                                $hasSeats    = $allElements->contains(fn($e) => ($e['type'] ?? '') === 'seat');
                                                                $hasExit     = $allElements->contains(fn($e) => ($e['type'] ?? '') === 'exit_row');
                                                                $firstSeat   = $allElements->firstWhere('type', 'seat');
                                                                $rowNum      = $firstSeat ? preg_replace('/[^0-9]/', '', $firstSeat['designator'] ?? '') : '';
                                                            @endphp
                                                            @if (! $hasSeats)
                                                                @if ($hasExit)
                                                                    <div wire:key="exit-{{ $cabinIdx }}-{{ $rowIdx }}" class="flex items-center gap-2 my-1 pl-10">
                                                                        <div class="h-0.5 w-4 bg-orange-300 rounded"></div>
                                                                        <span class="text-[10px] font-bold text-orange-400 uppercase tracking-widest">Exit Row</span>
                                                                        <div class="h-0.5 flex-1 bg-orange-300 rounded"></div>
                                                                    </div>
                                                                @endif
                                                                @continue
                                                            @endif
                                                            <div wire:key="row-{{ $cabinIdx }}-{{ $rowNum }}" class="flex items-center gap-1 mb-1.5">
                                                                <div class="w-8 shrink-0 flex items-center justify-end pr-1">
                                                                    <span class="font-semibold text-sm text-slate-500">{{ $rowNum }}</span>
                                                                </div>
                                                                @foreach ($sections as $secIdx => $section)
                                                                    ...
                                                                    @foreach ($section['elements'] ?? [] as $elIdx => $el)
                                                                        ...
                                                                        @php
                                                                            ...
                                                                            if ($activeDuffelId) {
                                                                                $activeSvc = collect($svcList)->first(fn($s) => ($s['passenger_id'] ?? '') === $activeDuffelId);
                                                                            } else {
                                                                                $activePaxIdx = $passengerMeta[$activePassengerKey]['index'] ?? 0;
                                                                                $activeSvc    = $svcList[$activePaxIdx] ?? ($svcList[0] ?? null);
                                                                            }

                                                                            $isAvailable  = ! empty($activeSvc);
                                                                            $activeSel    = $selectedSeats[$activeMapIndex][$activePassengerKey] ?? null;
                                                                            $isSelectedMe = $activeSel && $activeSel['designator'] === $designator;

                                                                            $takenByOther = false;
                                                                            $takenByName  = '';
                                                                            foreach ($selectedSeats[$activeMapIndex] as $pk => $seat) {
                                                                                if ($pk !== $activePassengerKey && $seat && $seat['designator'] === $designator) {
                                                                                    $takenByOther = true;
                                                                                    $takenByName  = $passengerMeta[$pk]['name'] ?? '';
                                                                                    break;
                                                                                }
                                                                            }

                                                                            $svcId  = $activeSvc['id']           ?? '';
                                                                            $amount = (float) ($activeSvc['total_amount'] ?? 0);
                                                                            $cur    = $activeSvc['total_currency'] ?? $currency;
                                                                        @endphp
                                                                        @if ($takenByOther)
                                                                            <div wire:key="seat-{{ $activeMapIndex }}-{{ $designator }}-{{ $activePassengerKey }}"
                                                                                ...
                                                                                <i data-tabler="user" data-size="16" data-stroke="1.8"></i>
                                                                            </div>
                                                                        @elseif ($isSelectedMe)
                                                                            <button
                                                                                ...
                                                                                <i data-tabler="armchair" data-size="18" data-stroke="1.8"></i>
                                                                            </button>
                                                                        @elseif ($isAvailable)
                                                                            <button
                                                                                wire:key="seat-{{ $activeMapIndex }}-{{ $designator }}-{{ $activePassengerKey }}"
                                                                        ...
                                                                        @else
                                                                            <div wire:key="seat-{{ $activeMapIndex }}-{{ $designator }}-{{ $activePassengerKey }}"
                                                                                ...
                                                                                <i data-tabler="x" data-size="16" data-stroke="2"></i>
                                                                            </div>
                                                                        @endif
                                                                    @endforeach
                                                                @endforeach
                                            ...
                                            <div class="w-full xl:w-[220px] flex flex-col gap-4 shrink-0">
                                                <div class="bg-slate-50 rounded-2xl border border-slate-100 p-5">
                                                    <span class="font-semibold text-base text-slate-950 block mb-4">Seat option</span>
                                                    <div class="flex flex-col gap-3.5">
                                                        <div class="flex items-center gap-3">
                                                            <div class="w-9 h-9 flex justify-center items-center bg-slate-100 rounded-xl border border-slate-200 text-slate-300 shrink-0">
                                                                <i data-tabler="x" data-size="16" data-stroke="2"></i>
                                                            </div>
                                                            <span class="text-sm text-slate-700">Unavailable seat</span>
                                                        </div>
                                                        <div class="flex items-center gap-3">
                                                            <div class="w-9 h-9 flex justify-center items-center bg-[#dff0fb] rounded-xl border border-[#c5e4f5] text-slate-600 shrink-0">
                                                                <i data-tabler="armchair" data-size="18" data-stroke="1.8"></i>
                                                            </div>
                                                            <span class="text-sm text-slate-700">Available seat</span>
                                                        </div>
                                                        <div class="flex items-center gap-3">
                                                            <div class="w-9 h-9 flex justify-center items-center bg-[#f3b515] rounded-xl border-2 border-[#e0a512] text-white shrink-0">
                                                                <i data-tabler="armchair" data-size="18" data-stroke="1.8"></i>
                                                            </div>
                                                        ...
                                                        @if (count($passengerMeta) > 1)
                                                            <div class="flex items-center gap-3">
                                                                <div class="w-9 h-9 flex justify-center items-center bg-purple-100 rounded-xl border border-purple-200 text-purple-500 shrink-0">
                                                                    <i data-tabler="user" data-size="16" data-stroke="1.8"></i>
                                                                </div>
                                                                <span class="text-sm text-slate-700">Other passenger</span>
                                                            </div>
                                                        @endif
                                                    </div>
                                                </div>
                                                <div class="bg-slate-50 rounded-2xl border border-slate-100 p-5">
                                                    <span class="font-semibold text-base text-slate-950 block mb-4">Selections</span>
                                                    <div class="flex flex-col gap-3">
                                                        @foreach ($passengerMeta as $paxKey => $meta)
                                                            @php $pSeat = $selectedSeats[$activeMapIndex][$paxKey] ?? null; @endphp
                                                            <div wire:key="summary-{{ $paxKey }}" class="flex items-center justify-between gap-2">
                                                                <div class="flex items-center gap-2 min-w-0">
                                                                    <i data-tabler="user" class="text-slate-400 shrink-0" data-size="14"></i>
                                                                    <span class="text-sm text-slate-700 truncate">{{ $meta['name'] }}</span>
                                                                </div>
                                                                @if ($pSeat)
                                                                    <div class="flex items-center gap-1 shrink-0">
                                                                        <span class="px-2 py-0.5 rounded-lg bg-[#f3b515] text-white text-xs font-bold">
                                                                            {{ $pSeat['designator'] }}
                                                                        </span>
                                                                        @if (($pSeat['amount'] ?? 0) > 0)
                                                                            <span class="text-xs text-slate-500 whitespace-nowrap">{{ number_format($pSeat['amount'], 0) }}</span>
                                                                        @else
                                                                            <span class="text-xs text-green-600 font-semibold">Free</span>
                                                                        @endif
                                                                    </div>
                                                                @else
                                                                    <span class="text-xs text-slate-400 italic shrink-0">—</span>
                                                                @endif
                                                            </div>
                                                        @endforeach
                                                    </div>
                        ...
                        <div class="flex justify-between items-center mt-2">
                            <button onclick="history.back()" class="btn btn-white min-w-[100px] sm:min-w-[140px]">Back</button>
                            <div class="flex items-center gap-2 sm:gap-4">
                                <button wire:click="skipSeats" class="text-blue-600 font-bold hover:underline px-2 sm:px-6">
                                    <span wire:loading.remove wire:target="skipSeats">Skip</span>
                                    <span wire:loading wire:target="skipSeats" class="loading loading-spinner loading-xs"></span>
                                </button>
                            ...
                            @if ($logo || $airl)
                                ...
                                <div class="card p-4 flex flex-row items-center justify-between gap-4">
                                    <div class="flex flex-col items-start">
                                        <span class="font-semibold text-sm text-slate-950">{{ $dep ? \Carbon\Carbon::parse($dep)->format('h:i A') : '' }}</span>
                                        <span class="font-normal text-xs text-slate-500">{{ $o }}</span>
                                    </div>
                                    <div class="flex flex-col items-center gap-0.5 flex-1">
                                        <span class="font-normal text-xs text-slate-400">{{ $dur ? \Carbon\CarbonInterval::make($dur)->cascade()->forHumans() : '' }}</span>
                                        ...
                                        <span class="font-normal text-xs text-slate-400">{{ $stps === 0 ? 'Non-stop' : $stps . ' stop' . ($stps > 1 ? 's' : '') }}</span>
                                    </div>
                                    <div class="flex flex-col items-end">
                                        <span class="font-semibold text-sm text-slate-950">{{ $arr ? \Carbon\Carbon::parse($arr)->format('h:i A') : '' }}</span>
