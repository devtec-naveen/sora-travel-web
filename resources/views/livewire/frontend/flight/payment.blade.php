<div
    ...
    x-on:require-login.window="login_modal.showModal()"
>
                ...
                <div class="flex flex-col lg:flex-row gap-6 lg:gap-8 lg:items-start">
                    ...
                    <div class="flex-1 flex flex-col gap-5 min-w-0">
                        ...
                        <div class="flex flex-col gap-1.5">
                            <h1 class="font-semibold text-xl md:text-[24px] leading-tight text-slate-800">Complete Payment</h1>
                            <span class="font-normal text-sm md:text-base text-slate-500">Choose your preferred payment method</span>
                        </div>
                        ...
                        @guest
                            <div class="card p-4 flex items-start gap-3 border border-blue-100 bg-blue-50">
                                ...
                                <div class="flex-1 min-w-0">
                                    ...
                                    <p class="text-xs text-blue-600 mt-0.5">Please login or create an account to proceed.</p>
                                </div>
                            ...
                            <div class="p-4 md:p-5 flex flex-col gap-3">
                                ...
                                <label class="relative cursor-pointer group">
                                    <input type="radio" name="payment_method" value="card"
                                        wire:model.live="paymentMethod" class="sr-only" />
                                ...
                                <label class="relative cursor-pointer group">
                                    ...
                                    <div class="flex items-center justify-between p-3.5 md:p-4 rounded-xl border bg-white transition-all
                                        group-has-[:checked]:border-blue-500 group-has-[:checked]:bg-blue-50 border-slate-200">
                                        <div class="flex items-center gap-3 md:gap-4 min-w-0">
                                            <div class="w-10 h-10 rounded-xl bg-[#635bff] flex items-center justify-center shrink-0">
                                                <span class="text-white font-bold text-base">S</span>
                                            </div>
                                            <div class="flex flex-col gap-0.5 min-w-0">
                                                <span class="font-semibold text-sm md:text-base text-slate-900">Stripe</span>
                                                <span class="font-normal text-xs md:text-sm text-slate-500">Secure payment via Stripe</span>
                                            </div>
                                        </div>
                                        <div class="w-5 h-5 rounded-full border shrink-0 flex items-center justify-center ml-3 transition-all
                                            group-has-[:checked]:bg-[#f3b515] group-has-[:checked]:border-[#f3b515] border-slate-300">
                        ...
                        @if ($paymentMethod === 'stripe')
                            <div class="card p-5 md:p-6 flex flex-col items-center gap-4 text-center">
                                <div class="w-12 h-12 md:w-14 md:h-14 rounded-2xl bg-[#635bff] flex items-center justify-center">
                                    <span class="text-white font-bold text-xl md:text-2xl">S</span>
                                </div>
                                <div>
                                    <p class="font-semibold text-sm md:text-base text-slate-900">You will be redirected to Stripe</p>
                                    <p class="text-xs md:text-sm text-slate-500 mt-1">After clicking Pay, you'll be taken to Stripe's secure checkout page.</p>
                                </div>
                            </div>
                        @endif
                        ...
                        <div class="block lg:hidden card p-4 space-y-3">
                            ...
                            @if (($paxTypesMob['adult'] ?? 0) > 0)
                                <div class="flex justify-between text-sm">
                                    <span class="text-slate-600">Base Fare ({{ $paxTypesMob['adult'] }} {{ $paxTypesMob['adult'] > 1 ? 'Adults' : 'Adult' }})</span>
                                    <span class="text-slate-700 font-medium">{{ $currency }} {{ number_format($perPaxMob * $paxTypesMob['adult'], 2) }}</span>
                                </div>
                            @endif
                            @if (($paxTypesMob['child'] ?? 0) > 0)
                                <div class="flex justify-between text-sm">
                                    <span class="text-slate-600">Base Fare ({{ $paxTypesMob['child'] }} {{ $paxTypesMob['child'] > 1 ? 'Children' : 'Child' }})</span>
                                    <span class="text-slate-700 font-medium">{{ $currency }} {{ number_format($perPaxMob * $paxTypesMob['child'], 2) }}</span>
                                </div>
                            @endif
                            @if ($addonsTotal > 0)
                                <div class="flex justify-between text-sm">
                            ...
                            @else
                                <button
                                    ...
                                    <span wire:loading.remove wire:target="pay" class="flex items-center justify-center gap-2">
                                        <i data-tabler="lock" data-size="15"></i>
                                        <span class="whitespace-nowrap">Pay {{ $currency }} {{ number_format($grandTotal, 2) }}</span>
                                    </span>
                            ...
                            @if ($logo || $airl)
                                ...
                                <div class="card p-4 flex items-center justify-between gap-3">
                                    <div class="flex flex-col">
                                        <span class="font-semibold text-sm text-slate-950">{{ $dep ? \Carbon\Carbon::parse($dep)->format('H:i') : '' }}</span>
                                        <span class="text-xs text-slate-500">{{ $orig }}</span>
                                    </div>
                                    <div class="flex flex-col items-center gap-0.5 flex-1">
                                        <span class="text-xs text-slate-400">{{ $dur ? \Carbon\CarbonInterval::make($dur)->cascade()->forHumans(['parts'=>2]) : '' }}</span>
                                        ...
                                        <span class="text-xs text-slate-400">{{ $stps === 0 ? 'Non-stop' : $stps . ' stop' . ($stps > 1 ? 's' : '') }}</span>
                                    </div>
                                    <div class="flex flex-col items-end">
                                        <span class="font-semibold text-sm text-slate-950">{{ $arr ? \Carbon\Carbon::parse($arr)->format('H:i') : '' }}</span>
                            ...
                            <div class="card p-5 space-y-3.5">
                                ...
                                @if (($paxTypes['adult'] ?? 0) > 0)
                                    <div class="flex justify-between items-center">
                                        <span class="text-sm text-slate-700">Base Fare ({{ $paxTypes['adult'] }} {{ $paxTypes['adult'] > 1 ? 'Adults' : 'Adult' }})</span>
                                        <span class="text-sm text-slate-500">{{ $currency }} {{ number_format($perPax * $paxTypes['adult'], 2) }}</span>
                                    </div>
                                @endif
                                @if (($paxTypes['child'] ?? 0) > 0)
                                    <div class="flex justify-between items-center">
                                        <span class="text-sm text-slate-700">Base Fare ({{ $paxTypes['child'] }} {{ $paxTypes['child'] > 1 ? 'Children' : 'Child' }})</span>
                                        <span class="text-sm text-slate-500">{{ $currency }} {{ number_format($perPax * $paxTypes['child'], 2) }}</span>
                                    </div>
                                @endif
                                @if ($addonsTotal > 0)
                                    <div class="flex justify-between items-center">
